...
if (isset($_POST)) {
    ...
    include("bd.php") ;
  // $db = mysqli_select_db($conn,'myphplogin');
  $email= $_POST['email'];
  $mdp = $_POST['mdp'];
  $message = '';
...
$iduser = $user["iduser"];
$datesign = date('Y-m-d');
 $verif = "SELECT iduser  FROM attendance WHERE iduser ='$iduser' and datesign = '$datesign'" ;
...
// // s'il n'y a pas de resultats 
 ...
 $iduser = $user["iduser"];
  ...
  $insert = "INSERT INTO attendance SET iduser = $iduser,datesign = NOW(),timesign = NOW()";
  $conn->query($insert);
